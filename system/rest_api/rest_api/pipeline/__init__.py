...
def _load_pipeline(pipeline_config_path, pipeline_name):
    with open (Path(pipeline_config_path)) as config_file:
        pipeline_config_dict = yaml.load(config_file, Loader=yaml.FullLoader)
    # Load pipeline (if available)
    try:
        pipeline = Pipeline.load_from_config(pipeline_config_dict, pipeline_name=pipeline_name)
        ...
        logger.error("Error loading %s pipeline from %s. \n %s\n", pipeline_name, pipeline_config_path, e.message)
...
def setup_pipelines() -> Dict[str, Any]:
    ...
    query_pipeline, document_store = _load_pipeline(config.PIPELINE_CONFIG_PATH, config.QUERY_PIPELINE_NAME)
