    ...
    def __prepare_ElasticsearchDocumentStore():
        ...
        if not es.ping():
            ...
            status = subprocess.run(
                ['docker run -d -p 9200:9200 -e "discovery.type=single-node" elasticsearch:7.17.9'], shell=True
            )
...
def create_dpr_training_dataset(squad_data: dict, retriever: BaseRetriever, num_hard_negative_ctxs: int = 30, max_seq_len_passage: int = 512):
    ...
    for article in tqdm(squad_data, unit="article"):
        ...
        for paragraph in article["paragraphs"]:
            ...
            for question in paragraph["qas"]:
                ...
                try:
                    answer, context_trimmed = fit_context_in_max_len(context, answers[0], max_seq_len_passage)
                except TypeError:
                    print (answers, answer)
                positive_ctxs = [{"title": article_title, "text": context_trimmed, "passage_id": ""}]

                if not hard_negative_ctxs:
                    
                    logger.error(
                        "No retrieved hard negative candidates for article %s, with question %s", article_title, question["question"]
                    )

                elif not positive_ctxs:

                    logger.error(
                        "No retrieved positive ctx candidates for article %s, with question %s", article_title, question["question"]
                    )
...
def get_hard_negative_contexts(retriever: BaseRetriever, question: str, answers: List[str], n_ctxs: int = 20):
    top_k = 20
    if n_ctxs > top_k:
    ...
    list_hard_neg_ctxs = []
    retrieved_docs = retriever.retrieve(query=question, top_k=top_k, index="document")
