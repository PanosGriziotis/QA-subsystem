...
def plot_retrievers_eval_report(json_path):
    ...
    methods = ['bm25_&_ranker', 'embedding_retriever_&_ranker', 'dpr_&_ranker']
    retriever_types = methods  # Methods already include _&_ranker suffix
    metrics = ['recall_multi_hit', 'map', 'mrr', 'ndcg']  # Removed 'precision'
    ...
    method_titles = {
        'bm25_&_ranker': 'BM25',
        'embedding_retriever_&_ranker': 'Embedding Retriever',
        'dpr_&_ranker': 'DPR'
    }
    ...
    for i, metric in enumerate(metrics):
        ...
        for j, method in enumerate(methods):
            ...
            ax.plot(top_k_values, ranker_data, label=f"{method_titles[method]} & Ranker", linestyle='--', color=colors[j])
